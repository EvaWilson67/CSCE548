...
public class CareDao {
    ...
    public int insert(Care cObj) throws SQLException {
        ...
        try (Connection c = DbUtil.getConnection(url, user, pass);
            ...
            if (cObj.getLastSoilChange() != null) ps.setDate(2, Date.valueOf(cObj.getLastSoilChange())); else ps.setNull(2, Types.DATE);
            if (cObj.getLastWatering() != null) ps.setDate(3, Date.valueOf(cObj.getLastWatering())); else ps.setNull(3, Types.DATE);
    ...
    public int update(Care cObj) throws SQLException {
        ...
        try (Connection c = DbUtil.getConnection(url, user, pass);
             PreparedStatement ps = c.prepareStatement(sql)) {
            if (cObj.getLastSoilChange() != null) ps.setDate(1, Date.valueOf(cObj.getLastSoilChange())); else ps.setNull(1, Types.DATE);
            if (cObj.getLastWatering() != null) ps.setDate(2, Date.valueOf(cObj.getLastWatering())); else ps.setNull(2, Types.DATE);
